...
foreach($account as $account_name) {
    ...
    $sum_dm_pid = mysqli_query($con, "SELECT SUM(AS_Approved_Revenue_Budget) FROM dmcdm WHERE (Account_Identifier ='$account_name')");
    $sum_dm_pid = mysqli_fetch_row($sum_dm_pid);

    // sum for all accounts
    $sum_pid = mysqli_query($con, "SELECT SUM(AS_Approved_Revenue_Budget) FROM dmcdm");
...
$result = mysqli_query($con,"ALTER TABLE sum ADD COLUMN `Rank` INT NOT NULL AUTO_INCREMENT PRIMARY KEY;");
if (!$result) {
...
$trafo_total = mysqli_query($con, "SELECT * FROM dmcdm");
$num_trafo_total = mysqli_num_rows($trafo_total);

foreach($account as $account_name) {

    // Counting the PID's
    $total = mysqli_query($con, "SELECT * FROM dmcdm WHERE (Account_Identifier ='$account_name')");
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="css/rank.css?<?php echo time();?>">
    <!-- JS -->
    ...
    <div id="dbottom" class="sixteen wide column">
        ...
        <div class="ui grid">
            <div class="sixteen wide column">
                <?php include('includes/rankindex.php');?>
            </div>
    </div>
</div>
