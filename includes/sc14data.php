...
/*Selecting column and storing it into array*/
$sth = mysqli_query($con, "SELECT FY14_PvsS FROM accounts.coefficient");
$rows2 = array();
$rows2['name'] = 'Service Coefficient';
while($r2 = mysqli_fetch_assoc($sth)) {
    $rows2['data'][] = $r2['FY14_PvsS'];
}

/*Selecting column and storing it into array*/
$sth = mysqli_query($con, "SELECT FY14_PvsTS FROM accounts.coefficient");
$rows3 = array();
$rows3['name'] = 'TS Coefficient';
while($r3 = mysqli_fetch_assoc($sth)) {
    $rows3['data'][] = $r3['FY14_PvsTS'];
}

/*Selecting column and storing it into array*/
$sth = mysqli_query($con, "SELECT FY14_PvsAS FROM accounts.coefficient");
$rows4 = array();
$rows4['name'] = 'AS Coefficient';
while($r4 = mysqli_fetch_assoc($sth)) {
    $rows4['data'][] = $r4['FY14_PvsAS'];
}
